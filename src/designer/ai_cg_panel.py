...
class AICGPanel(QWidget):
    ...
    def init_ui(self):
        ...
        self.prompt_group.setLayout(desc_layout)
    ...
    def _show_item(self, item: CGPendingItem):
        ...
        prompt_text = item.prompt or default_prompt
        self.prompt_edit.setPlainText(prompt_text)
        self.mj_prompt_edit.setPlainText(prompt_text)
        self.path_edit.setText(self._resource_path(item.file_path or f"resources/images/cg/{item.cg_id}.png").as_posix())
    ...
    def _mj_choose_final_path(self):
        default_dir = self._project_root() / "resources" / "images" / "cg"
        default_dir.mkdir(parents=True, exist_ok=True)
        base = (self.current_item.cg_id if self.current_item else "cg")
        ...
        if file_path:
            ...
            # 兼容：写回 path_edit 便于 file_label/状态展示复用
    ...
    def _mj_choose_path(self, target_edit: QLineEdit, suffix: str = "_grid.png"):
        default_dir = self._project_root() / "resources" / "mj" / "cg"
        default_dir.mkdir(parents=True, exist_ok=True)
        base = (self.current_item.cg_id if self.current_item else "cg")
        file_path, _ = QFileDialog.getSaveFileName(
            self,
    ...
    def _toggle_mj_group(self):
        ...
        # Midjourney 三步模式下：提示词/保存路径/一键生成按钮不再需要
    ...
    def _load_flux_state(self, item: CGPendingItem):
    ...
    def _mj_get_client(self):
        if not self._ensure_agent():
            return None
        client = self.cg_agent.api_manager.get_midjourney_client()
        if not client:
            QMessageBox.warning(self, "提示", "Midjourney 客户端不可用，请检查 config/ai_config.yaml")
        return client

    def _load_mj_state(self, item: CGPendingItem):
    ...
    def _mj_imagine(self):
        ...
        if not save_path:
            ...
            default_dir = self._project_root() / "resources" / "mj" / "cg"
            default_dir.mkdir(parents=True, exist_ok=True)
            save_path = str(default_dir / f"{self.current_item.cg_id}_grid.png")
    ...
    def _mj_separate(self):
        ...
        if not save_path:
            default_dir = self._project_root() / "resources" / "mj" / "cg"
            default_dir.mkdir(parents=True, exist_ok=True)
            save_path = str(default_dir / f"{self.current_item.cg_id}_U{index}.png")
    ...
    def _generate_cg(self):
        ...
        def _do_work():
            return self.cg_agent.execute(task)
